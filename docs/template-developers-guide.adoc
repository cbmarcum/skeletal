...
their contents and also reference our sample in https://github.com/cbmarcum/skeletal/tree/main/lazybones-templates/templates/java-gradle[lazybones-templates/templates/java-gradle]
...
You can see the full `README.md` in https://github.com/cbmarcum/skeletal/blob/main/lazybones-templates/templates/java-gradle/README.md[java-gradle/README.md].
...
java-gradle-template,1.0,Skeletal Project,A simple Java Spock Gradle project template
----
...
meaning that your source templates could be https://mustache.github.io/[Mustache], https://velocity.apache.org/[Apache Velocity], or anything else. Of course, not every template engine has a Groovy implementation, but it's
often trivial to create an adapter `TemplateEngine` implementation.

For the following examples, we'll use our https://github.com/cbmarcum/groovy-handlebars-engine[Groovy Handlebars Engine] implementation of https://jknack.github.io/handlebars.java/[Handlebars.java].

The first step to using an alternative template engine is to include the implementation JAR in the post-install script for your project template. Lazybones uses Groovy's https://docs.groovy-lang.org/latest/html/documentation/grape.html#_add_a_dependency[@Grab] annotation for that:
...
The Handlebars engine JAR is in Maven Central that @Grab automatically searches. If you have the JAR hosted elsewhere, you'll need to use https://docs.groovy-lang.org/latest/html/documentation/grape.html#Grape-SpecifyAdditionalRepositories[@GrabResolver].

_As of Skeletal 0.19.0, Groovy Handlebars Engine is an included library, so @Grab is not required as it is already on the classpath.  Other alternative template engines will still require it._
...
`subtmpl-spock-test` being excluded from the `*AllTemplates` tasks.
...
lazybones {
    ...
    template "java-gradle" includes "spock-test"
}
----
...
lazybones {
    ...
    template "java-gradle" includes "spock-test", "controller", "repository"
}
----
