...
As you can see, the `pkg` and `cls` properties are mapped from the return
values of two `ask()` calls. This is standard post-install script behavior.
The interesting properties, `parentPackage` and `parentClassName`, are mapped from
...
Because of this, `parentParams` only exists for subtemplates. Only
`parentClassName` is used in the template so it is in the `prop` map where
