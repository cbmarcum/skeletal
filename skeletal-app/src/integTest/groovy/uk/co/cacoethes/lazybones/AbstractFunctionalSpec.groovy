...
/**
 * The super class for all functional tests. It provides support for running skeletal
...
abstract class AbstractFunctionalSpec extends Specification {
    ...
    /**
     * Runs a skeletal command. For example:
     *
     * <pre>runCommand(["create", "ratpack", "0.1", "ratapp"], baseWorkDir)</pre>
     *
     * It will look for skeletal in either the directory specified by the
     * {@code lazybones.installDir} system property, or {cwd}/build/install. It will
     * also pass in the current PATH environment variable to the skeletal process if
     ...
     * larger.
     * @return The exit code of the skeletal process.
     */
    int runCommand(List cmdList, File workDir, List inputs = [], boolean clearPrevious = true) {
        ...
        def lazybonesInstallDir = System.getProperty("lazybones.installDir") ?:
                System.getProperty("user.dir") + "/build/install/skeletal"
        def lzbExecutable = FilenameUtils.concat(lazybonesInstallDir, "bin/skeletal")
        if (windows) {
    ...
    /**
     * Reads the current version of Skeletal from a properties file and
    ...
    protected final String getCacheDirPath() {
        return System.getProperty("lazybones.cache.dir") ?:
                FilenameUtils.concat(System.getProperty('user.home'), ".skeletal/templates")
    }

    protected final String getConfigFilePath() {
        return System.getProperty("lazybones.config.file") ?:
                FilenameUtils.concat(System.getProperty('user.home'), ".skeletal/config.groovy")
    }
