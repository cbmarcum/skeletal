...
class CreateFunctionalSpec extends AbstractFunctionalSpec {
    ...
    void setup() {
        ...
        filesToDelete << new File(cacheDirPath, "aoo-addin-template-0.3.0.zip")
    }

    def "Create command installs a packaged template"() {
        when: "I run skeletal with the create command for the aoo-addin template"
    ...
    def "Create command installs a template from an HTTP URL"() {
        ...
        def packageUrl = "https://skeletal.s3.us-east-2.amazonaws.com/openoffice-templates/aoo-addin-template-0.3.0.zip"
        ...
        and: "It says that the latest version of the package is being installed in the target directory"
        output =~ /Creating project from template https:\/\/skeletal.s3.us-east-2.amazonaws.com\/openoffice-templates\/aoo-addin-template-0.3.0.zip \(latest\) in 'test-addin'/

    }

    def "Create command installs a template from a URL mapping"() {
        when: "I run skeletal with the create command using mapping for the aoo-addin template"
    ...
    def "skeletal creates git repository on --with-git"() {
    ...
    def "Create can install from cache without template being in repository"() {
        when: "I run skeletal with the create command for a template that's only in the cache"
    ...
    def "Create command displays usage when incorrect number of arguments are provided"() {
        when: "I run skeletal with the create command without an extra argument"
