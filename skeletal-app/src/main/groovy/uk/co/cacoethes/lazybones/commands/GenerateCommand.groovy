...
/**
 * Implements Skeletal's generate command, which processes subtemplates in a
 * Skeletal-created project. The command unpacks the subtemplate into the
...
"""
    ...
    protected int doExecute(OptionSet cmdOptions, Map globalOptions, Configuration configuration) {
        ...
        if (!LAZYBONES_DIR.exists()) {
            log.severe "You cannot use `generate` here: this is not a Skeletal-created project"
            return 1
        }
